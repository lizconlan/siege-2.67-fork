 ...
 * 
 * Liz Conlan - hacked to include count of 
 * transaction failures (i.e. where the socket
 * is fine but an unexpected status of 400 or
 * above was returned)
 */
void
...
void
write_to_log(int count, float elapsed, int bytes, float ttime, int code, int failed, int fails)
{
  int     fd; 
  char    entry[512];
  ...
  snprintf(
    entry, sizeof entry, 
    "%s,%7d,%11.2f,%12u,%11.2f,%12.2f,%12.2f,%12.2f,%8d,%8d,%8d\n", 
    date, count, elapsed, bytes, ttime / count, count / elapsed, bytes / elapsed, 
    ttime / elapsed, code, failed, fails
  );
...
create_logfile(const char *file)
{
  ...
  char *head = (char*)"      Date & Time,  Trans,  Elap Time,  Data Trans,  "
               "Resp Time,  Trans Rate,  Throughput,  Concurrent,    OKAY, Failed Socket, Failed Trans\n"; 
